<template>
  ...
  >
    <!--图片预览列表-->
    <div v-if="hasImages" class="img-uploader">
      <div class="img-uploader-preview-list">
        ...
        >
          ...
          <div class="img-uploader-mask" v-if="hasImages">
            ...
            <p class="img-uploader-file-name" @click="openInput()">
              {{ fileNameList[index] }}
            </p>
          </div>
          <img
      ...
      <!--文件数-->
      <div class="img-uploader-count" v-if="countText.length">
        {{ countText }}
      </div>
      <v-btn text color="primary accent-4">提交</v-btn>
      <v-btn text color="primary accent-4" @click="imageDataList = []"
        >取消</v-btn
      >
    </div>

    <div v-else>
      <v-btn text color="primary accent-4">选择图片</v-btn>
    </div>
...
<script>
import resizeImage from '@/assets/resize'
